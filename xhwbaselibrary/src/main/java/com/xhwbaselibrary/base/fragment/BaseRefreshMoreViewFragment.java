...
public abstract class BaseRefreshMoreViewFragment extends BaseFragment {
     ...
     */
    protected abstract BaseRefreshMoreViewActivity.RefreshReturnType doStartTask();
    /**
     * 开始执行刷新的任务
     * @return
     */
...
//        mListView.addItemDecoration(decoration);
    ...
    private AutoPullToRefreshView.OnRefreshListener onRefreshListener=new AutoPullToRefreshView.OnRefreshListener(){
        ...
        public void onRefresh() {
            ...
            BaseRefreshMoreViewActivity.RefreshReturnType type = doStartTask();
            if (type == BaseRefreshMoreViewActivity.RefreshReturnType.NONE) {
                refresh_layout.setRefreshing(false);
            } else if (type == BaseRefreshMoreViewActivity.RefreshReturnType.UI_THREAD||type==null) {
                doRefreshTask();
            } else {
    ...
    protected void showLoadingFootView(int maxLength) {
        RecyclerViewStateUtils.setFooterViewStateNoScroll(getActivity(), mListView, maxLength, LoadingFooter.State.Loading, null);
    }
    ...
    protected void showEndingFootView(int maxLength) {
        RecyclerViewStateUtils.setFooterViewStateNoScroll(getActivity(), mListView, maxLength, LoadingFooter.State.TheEnd, null);
    }
    ...
    protected void showErrorFootView(int maxLength) {
        RecyclerViewStateUtils.setFooterViewStateNoScroll(getActivity(), mListView, maxLength, LoadingFooter.State.NetWorkError, null);
    }
